// --- DYNAMIC CUSTOM QUESTS ENGINE ---

// Form Submission Event Listener Hook
document.addEventListener('DOMContentLoaded', () => {
    const questForm = document.getElementById('quest-form');
    if (questForm) {
        questForm.addEventListener('submit', createCustomQuest);
    }
});

// Create a new quest object and add it to game state
function createCustomQuest(event) {
    event.preventDefault();

    // Get the actual values at the moment of clicking
    const title = document.getElementById('q-title').value.trim();
    
    // validation: Don't add if the title is empty!
    if (!title) return;
    ...
    // 2. Save to "browser database"
    Storage.save(window.gameState);
    
    // 3. Draw the new list
    renderCustomQuests();
    
    // 4. Clear the form ONLY ONCE
    event.target.reset(); 
}

// Render custom arrays out to HTML structures
function renderCustomQuests() {
    ...
    if (!listContainer) return; // Exit if not currently on quests.html page
    ...
    sortedQuests.forEach(quest => {
        ...
        // Dynamic border color styling configuration blocks depending on difficulty tiers
...
// Checkbox interactions trigger state logic execution updates
...
// Complete removal execution blocks 
function deleteQuest(id) {
    ...
    // Deduct completed stats points if we delete a quest that was already checked off
    if (quest.completed) {
        gainXp(-quest.xp);
        window.gameState.questsCompleted = Math.max(0, window.gameState.questsCompleted - 1);
    }
...
// --- CLASSIC DASHBOARD ENGINE HOOKS (From original dashboard build setup) ---
...
// --- ACHIEVEMENT CORE SCANNING ENGINE ---
function checkAchievements(silent = false) {
    let stateChanged = false;

    window.achievementDefinitions.forEach(ach => {
        // Skip assessment loops if the milestone is already marked unlocked
        ...
        if (conditionMet) {
            ...
            // Fire the visual popup if it's a live event, not an initial page load check
            if (!silent) {
...
// Overwrite your existing main modifier functions to catch active progression updates:
function toggleCustomQuest(id, checkbox) {
    const quest = window.gameState.customQuests.find(q => q.id === id);
    if (!quest) return;
    ...
    checkAchievements(false); // Evaluate milestones dynamically!
    Storage.save(window.gameState);
    UI.update(window.gameState);
    renderCustomQuests();
}
...
function claimQuestXp(xpValue, checkbox) {
    ...
    // THE ADDITION: Check for milestones after stats update
...
function gainXp(amount) {
    ...
    checkAchievements(false); // Catch level shifts
}